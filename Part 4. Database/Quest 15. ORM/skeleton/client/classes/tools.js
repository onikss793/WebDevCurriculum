...
class SaveFile {
    ...
    async _postFile(data) {
        try {
            const response = await this.API.saveNote(data);
...
class LoadFile {
    ...
    _getFiles = async () => {
        try {
            const response = await this.API.getNotesList();
            ...
            if (this._checkResposne(response)) {
                ...
                if (answer) {
                    const id = this._renderId(answer);
                    const response = await this.API.getNote(id);
